...
*/
      ...
      // bool 
char fdtdGPU(float *output, const float *input, const float *coeff, const int dimx, const int dimy, const int dimz, const int radius, const int timesteps, const int argc, const char **argv)
{
    ...
    */
    ...
    bufferOut = (float*) malloc(paddedVolumeSize * sizeof(float)); 
    // checkCudaErrors(cudaMalloc((void **)&bufferIn, paddedVolumeSize * sizeof(float)));
    bufferIn = (float*) malloc(paddedVolumeSize * sizeof(float));
