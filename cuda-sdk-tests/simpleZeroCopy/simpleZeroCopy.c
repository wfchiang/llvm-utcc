...
int main(int argc, char **argv)
{
    ...
    */
    ...
    // dim3 block(256);
    NonNegativeXYZCoordinates block = {1, 1, 256}; 
    // dim3 grid((unsigned int)ceil(nelem/(float)block.x));
    NonNegativeXYZCoordinates grid = {1, 1, 
				      ((unsigned int)ceil(nelem/(float)block.x))}; 
    // vectorAddGPU<<<grid, block>>>(d_a, d_b, d_c, nelem);
    vectorAddGPU(d_a, d_b, d_c, nelem);
