...
#define __saturatef(fp) max(0, min(1, fp)) 

#define SIZE_TEXARRAY 4096
// texture<float, 2> tex;
float tex[SIZE_TEXARRAY][SIZE_TEXARRAY]; 
// texture<uchar4, 2, cudaReadModeNormalizedFloat> rgbaTex;
uchar4 rgbaTex[SIZE_TEXARRAY][SIZE_TEXARRAY]; 
...
unsigned int rgbaFloatToInt(float4 rgba)
{
  ...
  */
    fx = __saturatef(rgba.x);   // clamp to [0.0, 1.0]
    fy = __saturatef(rgba.y);
    fz = __saturatef(rgba.z);
    fw = __saturatef(rgba.w);
    return ((unsigned int)(fw * 255.0f) << 24) |
           ((unsigned int)(fz * 255.0f) << 16) |
           ((unsigned int)(fy * 255.0f) <<  8) |
           ((unsigned int)(fx * 255.0f));
}

// __device__ 
float4 rgbaIntToFloat(unsigned int c)
{
    float4 rgba;
...
d_boxfilter_rgba_x(unsigned int *od, int w, int h, int r)
{
  ...
  if (y < h)
    {
      float4 t = {0.0, 0.0, 0.0, 0.0}; // make_float4(0.0f);

      for (x = -r; x <= r; x++)
        {
	  // t += tex2D(rgbaTex, x, y);
	  t = float4uchar4Add(t, rgbaTex[x][y]); 
        }
      
      // od[y * w] = rgbaFloatToInt(t * scale);
      od[y * w] = rgbaFloatToInt(float4scalarMul(t, scale));
      
      for (x = 1; x < w; x++)
        {
	  // t += tex2D(rgbaTex, x + r, y);
	  t = float4uchar4Add(t, rgbaTex[x + r][y]); 
	  // t -= tex2D(rgbaTex, x - r - 1, y);
	  t = float4uchar4Add(t, rgbaTex[x - r - 1][y]); 

	  // od[y * w + x] = rgbaFloatToInt(t * scale);
	  od[y * w + x] = rgbaFloatToInt(float4scalarMul(t, scale)); 
        }
    }
}
...
d_boxfilter_rgba_y(unsigned int *id, unsigned int *od, int w, int h, int r)
{
  ...
  t = float4scalarMul(rgbaIntToFloat(id[0]), r);
  
  for (y = 0; y < (r + 1); y++)
    {
      // t += rgbaIntToFloat(id[y*w]);
      t = float4float4Add(t, rgbaIntToFloat(id[y*w]));
    }
  
  // od[0] = rgbaFloatToInt(t * scale);
  od[0] = rgbaFloatToInt(float4scalarMul(t, scale));
  
  
  for (y = 1; y < (r + 1); y++)
    {
      // t += rgbaIntToFloat(id[(y + r) * w]);
      t = float4float4Add(t, rgbaIntToFloat(id[(y + r) * w]));
      // t -= rgbaIntToFloat(id[0]);
      t = float4float4Sub(t, rgbaIntToFloat(id[0])); 
      // od[y * w] = rgbaFloatToInt(t * scale);
      od[y * w] = rgbaFloatToInt(float4scalarMul(t, scale));
    }
  ...
  for (y = (r + 1); y < (h - r); y++)
    {
      ...
      t = float4float4Sub(t, rgbaIntToFloat(id[((y - r) * w) - w]));
      // od[y * w] = rgbaFloatToInt(t * scale);
      od[y * w] = rgbaFloatToInt(float4scalarMul(t, scale));
    }
  ...
  for (y = h - r; y < h; y++)
    {
      ...
      t = float4float4Add(t, rgbaIntToFloat(id[(h - 1) * w])); 
      // t -= rgbaIntToFloat(id[((y - r) * w) - w]);
      t = float4float4Sub(t, rgbaIntToFloat(id[((y - r) * w) - w])); 
      // od[y * w] = rgbaFloatToInt(t * scale);      
      od[y * w] = rgbaFloatToInt(float4scalarMul(t, scale));
...
void initTexture(int width, int height, void *pImage, char useRGBA)
{
  ...
  for (i=0; i<iterations; i++)
    {
      ...
      d_boxfilter_x_tex(d_temp, width, height, radius);
      d_boxfilter_y_global(d_temp, d_dest, width, height, radius);
  ...
  for (i=0; i<iterations; i++)
    {
      ...
      // use texture for horizontal pass
      d_boxfilter_rgba_x(d_temp, width, height, radius);
      d_boxfilter_rgba_y(d_temp, d_dest, width, height, radius);
