...
#define __expf exp
#define expf exp 
#define __saturatef(fp) min(0, max(1, fp))

#define SIZE_TEXARRAY 4096 
// __constant__ 
float cGaussian[64];   //gaussian array in device side
// texture<uchar4, 2, cudaReadModeNormalizedFloat> rgbaTex;
uchar4 rgbaTex[SIZE_TEXARRAY][SIZE_TEXARRAY]; 

uint *dImage  = 0; // NULL;   //original image
uint *dTemp   = 0; // NULL;   //temp array for iterations
size_t pitch;
                   ...
                   float e_d,  int r)
{
    ...
    // float4 center = rgbaTex[x][y]; 
    float4 center = {rgbaTex[x][y].w, 
		     rgbaTex[x][y].x, 
		     rgbaTex[x][y].y, 
		     rgbaTex[x][y].z}; 
    ...
    for (i = -r; i <= r; i++)
    {
	  ...
	  // float4 curPix = rgbaTex[x + j][y + i];	  
	  float4 curPix = {rgbaTex[x + j][y + i].w, 
			   rgbaTex[x + j][y + i].x, 
			   rgbaTex[x + j][y + i].y, 
			   rgbaTex[x + j][y + i].z}; 
	  ...
	  // t += factor * curPix;
	  t = float4float4Add(t, float4scalarMul(curPix, factor)); 
	  sum += factor;
        }
    }

    // od[y * w + x] = rgbaFloatToInt(t/sum);
    od[y * w + x] = rgbaFloatToInt(float4scalarDiv(t, sum));
}
                           ...
                           float e_d, int radius, int iterations) 
{
    ...
    for (i=0; i<iterations; i++)
    {
      ...
      */
      ...
      NonNegativeXYZCoordinates gridSize  = {1, 
					     ((width + 16 - 1) / 16, 
					      (height + 16 - 1) / 16)}; 
      // dim3 blockSize(16, 16);
      NonNegativeXYZCoordinates blockSize = {1, 16, 16}; 
      // d_bilateral_filter<<< gridSize, blockSize>>>(dDest, width, height, e_d, radius);
      d_bilateral_filter(dDest, width, height, e_d, radius);
